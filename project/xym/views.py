...
def my_new_house(request):
    ...
    if request.method == 'POST':
        ...
        user_id = 1
        house = House.objects.create( # this creates new house in table, house id is automatically created for this object
            user_id=user_id,
            area_id=area_id,
            type_id=type_id,
            title=title,
        ...
        # my_house = request.POST.get('house_id')
        # house_id = House.objects.filter(house_id=my_house).first()
        house_detail = HouseDetail.objects.create( # another table ,is not house table, the house detail need house id  to connect
            house_id=house_id,
            lease=lease,
            pay_way=pay_way,
            floor=floor,
